...
const options = {
  ...
  templates: {
    app: ({ head, body, assets, nonce, env }) => '<!DOCTYPE html>\r\n<html lang="en">\r\n	<head>\r\n		<meta charset="utf-8" />\r\n		<link rel="icon" href="' + assets + `/favicon.png" />\r
		...
		<style>\r
			html[data-locale-pending='true'] body {\r
				visibility: hidden;\r
			}\r
		</style>\r
		<script>\r
			(() => {\r
				const DEFAULT_LOCALE = 'en';\r
				const STORAGE_KEY = 'preferred-locale';\r
				const root = document.documentElement;\r
				let locale = DEFAULT_LOCALE;\r
\r
				try {\r
					const storedUser = JSON.parse(localStorage.getItem('user') || 'null');\r
					const preferredLocale = storedUser?.preferences?.language;\r
					if (preferredLocale === 'en' || preferredLocale === 'bn') {\r
						locale = preferredLocale;\r
					} else {\r
						const storedLocale = localStorage.getItem(STORAGE_KEY);\r
						if (storedLocale === 'en' || storedLocale === 'bn') {\r
							locale = storedLocale;\r
						}\r
					}\r
				} catch (_error) {\r
					locale = DEFAULT_LOCALE;\r
				}\r
\r
				root.lang = locale === 'bn' ? 'bn-BD' : 'en';\r
				root.dataset.locale = locale;\r
				root.dataset.localeBoot = locale;\r
\r
				if (locale === 'bn') {\r
					root.dataset.localePending = 'true';\r
					window.setTimeout(() => {\r
						if (root.dataset.localePending === 'true') {\r
							root.dataset.localePending = 'false';\r
						}\r
					}, 4000);\r
				}\r
\r
				window.__NB_LOCALE__ = locale;\r
			})();\r
		<\/script>\r
