...
function DifficultyBadge($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="difficulty-badge svelte-1acasio">`);
    if (domain) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<span class="domain-label svelte-1acasio">${escape_html(domainLabel)}</span> <span class="separator svelte-1acasio">•</span>`);
    } else {
      $$renderer2.push("<!--[!-->");
    }
    $$renderer2.push(`<!--]--> <span class="level-text svelte-1acasio">${escape_html(difficultyLabel)} ${escape_html(displayDifficulty)}</span> <span class="level-label svelte-1acasio">(${escape_html(label)})</span></div>`);
