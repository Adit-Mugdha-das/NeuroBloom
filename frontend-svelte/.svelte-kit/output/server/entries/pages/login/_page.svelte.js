...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="auth-container"><div class="auth-card"><h1>Welcome Back</h1> <p>Login to continue your cognitive training</p> <div class="login-type-selector svelte-1btfugp"><button type="button"${attr_class(`type-btn ${stringify("active")}`, "svelte-1btfugp")}${attr("disabled", loading, true)}>👤 ${escape_html(translateText("Patient", store_get($$store_subs ??= {}, "$locale", locale)))}</button> <button type="button"${attr_class(`type-btn ${stringify("")}`, "svelte-1btfugp")}${attr("disabled", loading, true)}>👨‍⚕️ ${escape_html(translateText("Doctor", store_get($$store_subs ??= {}, "$locale", locale)))}</button> <button type="button"${attr_class(`type-btn ${stringify("")}`, "svelte-1btfugp")}${attr("disabled", loading, true)}>🏥 ${escape_html(translateText("Admin", store_get($$store_subs ??= {}, "$locale", locale)))}</button></div> `);
    {
      $$renderer2.push("<!--[!-->");
    }
