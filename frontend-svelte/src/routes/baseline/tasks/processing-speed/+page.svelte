...
onMount(() => {
	...
	[simpleTrials, choiceTrials, delayMin, delayMax, earlyClickPenalty, choiceShapeCount] = [st, ct, dMin, dMax, penalty, shapeCount];
});
...
alert('Too early! Wait for the green screen. Try again.');
nextSimpleTrial();
		} else {
			simpleRTs.push(9999);
			alert(`Too early! (Early clicks: ${earlyClickCount})`);
			simpleCurrentTrial++;
			nextSimpleTrial();
		}
} else if (simpleReady) {
...
<li>You'll see a <strong>Circle ⭕</strong> or <strong>Square ⬜</strong></li>
{:else if choiceShapeCount === 3}
<li>You'll see <strong>Circle ⭕</strong>, <strong>Square ⬜</strong>, or <strong>Triangle 🔺</strong></li>
{:else}
<li>You'll see <strong>Circle ⭕</strong>, <strong>Square ⬜</strong>, <strong>Triangle 🔺</strong>, or <strong>Diamond 🔶</strong></li>
{/if}
...
<button class="btn-primary" on:click={startSimpleTest} style="margin-top: 40px;">Start Test</button>
<button class="btn-secondary" on:click={backToDashboard}>Back to Dashboard</button>
</div>
...
<div class="test-card">
<div class="timer">Trial {choiceCurrentTrial + 1} / {choiceTrials}</div>
...
<div style="width: 0; height: 0; border-left: 75px solid transparent; border-right: 75px solid transparent; border-bottom: 130px solid #4caf50; margin: 0 auto;"></div>
{:else if choiceCurrentShape === 'diamond'}
<div style="width: 106px; height: 106px; background: #9c27b0; transform: rotate(45deg); margin: 22px auto;"></div>
{/if}
</div>
<div style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap; max-width: 600px; margin: 40px auto 0;">
<button class="btn-primary" on:click={() => handleChoiceResponse('circle')} style="font-size: 16px; padding: 15px 30px; min-width: 140px;">⭕ Circle</button>
<button class="btn-primary" on:click={() => handleChoiceResponse('square')} style="font-size: 16px; padding: 15px 30px; min-width: 140px;">⬜ Square</button>
{#if choiceShapeCount >= 3}
<button class="btn-primary" on:click={() => handleChoiceResponse('triangle')} style="font-size: 16px; padding: 15px 30px; min-width: 140px;">🔺 Triangle</button>
{/if}
{#if choiceShapeCount >= 4}
<button class="btn-primary" on:click={() => handleChoiceResponse('diamond')} style="font-size: 16px; padding: 15px 30px; min-width: 140px;">🔶 Diamond</button>
{/if}
</div>
</div>
...
{#if sessionComplete}
<div class="session-complete-msg">🎉 Session Complete! All 4 tasks finished.</div>
{:else}
...
{/if}
<div class="score-display">{finalScore.toFixed(1)}%</div>
<div class="result-details">
<p style="background: #e3f2fd; padding: 15px; border-radius: 6px;"><strong style="color: #2196f3;">Part 1: Simple Reaction Time</strong></p>
<p><span>Average RT:</span><strong>{(validSimpleRTs.reduce((a, b) => a + b, 0) / validSimpleRTs.length || 0).toFixed(0)}ms</strong></p>
<p><span>Consistency:</span><strong>{calculateStd(validSimpleRTs).toFixed(0)}ms</strong></p>
<p><span>Valid Trials:</span><strong>{validSimpleRTs.length}/{simpleTrials}</strong></p>
{#if earlyClickCount > 0}
<p style="color: #ff9800;"><span>Early Clicks:</span><strong>{earlyClickCount}</strong></p>
{/if}
<p style="background: #fff3e0; padding: 15px; border-radius: 6px; margin-top: 20px;"><strong style="color: #ff9800;">Part 2: Choice Reaction Time</strong></p>
<p><span>Average RT:</span><strong>{(choiceRTs.reduce((a, b) => a + b, 0) / choiceRTs.length || 0).toFixed(0)}ms</strong></p>
<p><span>Accuracy:</span><strong>{((choiceAccuracy.filter(a => a).length / choiceAccuracy.length) * 100).toFixed(1)}%</strong></p>
<p><span>Correct:</span><strong>{choiceAccuracy.filter(a => a).length}/{choiceTrials}</strong></p>
</div>
<div style="margin-top: 40px;">
<button class="btn-primary" on:click={backToDashboard}>Back to Dashboard</button>
</div>
</div>
{/if}
</div>
