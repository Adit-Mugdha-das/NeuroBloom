...
<div class="admin-layout">
	...
	<main class="main-content">
		<header class="topbar">
			<div>
				<h1 class="page-title">{uiText("Research Data Exports", $activeLocale)}</h1>
				<p class="page-sub">{uiText("Export anonymized NeuroBloom datasets for research papers, secondary analysis, and institutional reporting.", $activeLocale)}</p>
			</div>
		...
		<div class="content">
			...
			{#if loading}
				<div class="loading-state">{uiText("Loading research export catalog...", $activeLocale)}</div>
			{:else}
				<div class="overview-grid">
					<div class="overview-card hero-card">
						<p class="hero-kicker">{uiText("Research Ready", $activeLocale)}</p>
						<h2>{uiText("Anonymized datasets for publication workflows", $activeLocale)}</h2>
						<p>{uiText("All exports remove direct patient identifiers and support CSV, JSON, and concise PDF research reports.", $activeLocale)}</p>
					</div>
					<div class="overview-card stat-card">
						<p class="stat-value">{formatNumber(summary?.total_datasets)}</p>
						<p class="stat-label">{uiText("Datasets", $activeLocale)}</p>
					</div>
					<div class="overview-card stat-card">
						<p class="stat-value">{formatNumber(summary?.total_records)}</p>
						<p class="stat-label">{uiText("Rows Available", $activeLocale)}</p>
					</div>
				</div>
					...
					{#each datasets as dataset}
						<section class="dataset-card">
							<div class="dataset-head">
								...
								<span class="dataset-count">{formatNumber(dataset.record_count)} {uiText("rows", $activeLocale)}</span>
							</div>
							...
							<div class="dataset-metrics">
								<div class="metric-pill">
									<span>{uiText("Patients", $activeLocale)}</span>
									<strong>{formatNumber(dataset.unique_patients)}</strong>
								</div>
								<div class="metric-pill">
									<span>{uiText("Start", $activeLocale)}</span>
									<strong>{formatDate(dataset.date_range?.start)}</strong>
								</div>
								<div class="metric-pill">
									<span>{uiText("Latest", $activeLocale)}</span>
							...
							{#if dataset.preview_fields?.length}
								<div class="preview-block">
									<p class="preview-label">{uiText("Preview fields", $activeLocale)}</p>
