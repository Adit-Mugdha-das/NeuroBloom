...
\begin{abstract}
NeuroBloom is a modular digital health platform developed to support longitudinal cognitive monitoring and clinician-guided rehabilitation for people living with multiple sclerosis (MS). The platform combines patient-facing cognitive tasks, pre-session contextual questionnaires, digital biomarker extraction, clinician review tools, prescription and training-plan management, and administrative oversight within a role-based architecture. It is intended for repeated, low-cost follow-up between formal clinical visits, with a focus on identifying subtle changes in performance over time rather than making diagnostic claims. By integrating structured task execution, longitudinal analytics, and coordinated patient-doctor workflows, NeuroBloom aims to support accessible and reproducible digital rehabilitation research and practice in MS care.
\end{abstract}
...
Several elements of NeuroBloom build on established neuropsychological practice, including the use of task paradigms relevant to processing speed, sustained attention, working memory, and executive function \cite{benedict2012,kalb2018}. However, the contribution of the present software is not the introduction of a new cognitive test in isolation. Instead, NeuroBloom provides a unified digital environment that links task execution, contextual capture, longitudinal analytics, clinician-guided rehabilitation, and multi-role workflow management in a single platform. This makes the software relevant to future work on accessible MS monitoring, remote rehabilitation support, digital biomarker exploration, and reproducible cognitive follow-up across time in both research and practice.

\section{Software description}

NeuroBloom provides an integrated web-based environment for longitudinal cognitive monitoring and clinician-guided rehabilitation support in multiple sclerosis. The platform brings together patient-side task execution, contextual data collection, clinician review workflows, administrative oversight, and analytics-driven interpretation within a unified system. At a broader level, it is intended to support supervised cognitive follow-up while remaining extensible to future task modules, reporting features, and research-oriented analytical components

\subsection{Software architecture}
The internal organization of NeuroBloom is based on a modular, role-aware architecture that separates interface functions, application services, analytics processing, and persistent storage. The platform is structured around three main operational contexts—patient, doctor, and administrator—which share a common backend and data model while maintaining role-specific workflows. This arrangement preserves coordinated interaction across users while keeping clinical supervision, patient activity, and administrative control clearly separated.

At implementation level, NeuroBloom combines a Svelte-based frontend, a FastAPI backend, and a persistence layer built on SQLModel and PostgreSQL. The frontend delivers role-specific interfaces for task execution, progress review, patient management, analytics, and administration. The backend handles authentication, baseline and training operations, reporting, messaging, notifications, and doctor-admin coordination. This layered organization separates interface behavior from service logic and storage responsibilities, making the system easier to manage and adapt as new task modules or platform features are introduced.
...
Within this design, data flow begins when a patient completes baseline or training activities and, when applicable, submits contextual information such as fatigue, sleep quality, stress, or medication timing. These records pass through backend services responsible for session creation, task submission, history management, and controlled access for clinician and administrator views. Stored session data include measures such as score, accuracy, response time, completion behavior, and associated contextual variables. This allows patient performance to be reviewed across sessions rather than treated as a series of isolated task results.

An analytics layer operates on top of these stored records to generate summary measures and behavioral indicators from accumulated interaction data. Rather than acting as a standalone diagnostic engine, it functions as a monitoring pipeline that aggregates task outputs, derives variability- and fatigue-related measures, evaluates change across sessions, and returns interpretable summaries for clinician-facing review. The processing sequence from cognitive task interaction to biomarker-oriented and time-based summaries is illustrated in Fig.~\ref{fig:biomarker_pipeline}.
...
The separation of interface layer, application services, analytics processing, and persistence is important for both software quality and research utility. It enables controlled role-based access, supports reuse of shared data structures, and allows the platform to evolve without requiring redesign of the full system. In the setting of MS-oriented monitoring and rehabilitation support, the architecture therefore serves not only as a technical foundation, but also as a practical basis for reproducible data capture and coordinated clinician-patient interaction.


\subsection{Software functionalities}
\paragraph{Patient interaction and session execution}
NeuroBloom provides a patient-facing workflow for baseline and training activity across multiple sessions rather than one-time task use. Patients can complete structured cognitive activities, view session progress, and optionally submit pre-session contextual information such as fatigue or sleep quality before task execution. In this way, the software supports adherence-oriented use over time while preserving a simple interaction flow for repeated home- or clinic-based follow-up. The patient-facing interaction flow is illustrated in Fig.~\ref{fig:patient_workflow}.
...
The task framework currently includes 34 tests distributed across six cognitive domains: working memory, processing speed, attention, cognitive flexibility, planning, and visual scanning. Although individual tasks differ in presentation and interaction style, they are integrated into a common session model that standardizes outputs including score, accuracy, response speed, completion behavior, and difficulty-linked progression. This allows heterogeneous task designs to contribute to a shared record across sessions instead of remaining isolated task-specific measurements.

Each task is implemented across 10 difficulty levels, enabling adaptive progression based on patient performance. NeuroBloom also incorporates a session-level task selection mechanism that determines which cognitive domains should be emphasized in a given session and selects four domain tasks accordingly, with priority given to weaker or more rehabilitation-relevant areas.
...
Beyond raw task completion, the platform supports session-based monitoring through summary analytics derived from accumulated performance and contextual records. These outputs include longitudinal trend summaries, fatigue-related patterns, response-time variability, and related indicators that help characterize change over time at patient level. Their purpose within NeuroBloom is to support progression-aware monitoring and clinician interpretation of rehabilitation-relevant performance change rather than to provide diagnostic conclusions. The processing pipeline underlying these analytics-oriented outputs is illustrated in Fig.~\ref{fig:biomarker_pipeline}, while an example dashboard view of derived biomarker and contextual summaries is shown in Fig.~\ref{fig:admin_workflow}(a).

The current analytics layer is based on deterministic statistical feature extraction rather than machine-learning prediction models. All indicators are computed directly from trial-level accuracy, reaction times, session-level scores, and contextual self-report variables. Within-session cognitive fatigue is estimated from the difference between early- and late-session performance, with
\begin{equation}
...
Reaction-time variability is summarized using standard dispersion measures, with coefficient of variation computed as
\begin{equation}
CV = \frac{\sigma_{RT}}{\mu_{RT}},
\end{equation}
where $\sigma_{RT}$ and $\mu_{RT}$ are the standard deviation and mean of valid reaction times. For longitudinal monitoring, NeuroBloom applies an exponentially weighted moving average (EWMA) to repeated performance values:
\begin{equation}
EWMA_t = \alpha x_t + (1-\alpha)EWMA_{t-1},
\end{equation}
with $\alpha=0.2$ in the current implementation. Across sessions, score change is also summarized using a reliable change index,
\begin{equation}
RCI = \frac{X_{\mathrm{latest}} - X_{\mathrm{baseline}}}{\sqrt{2}\,\sigma_X},
\end{equation}
where $\sigma_X$ is the observed standard deviation of session scores for the individual. Associations between contextual variables such as fatigue level, sleep quality, and hours since medication are estimated using Pearson correlation coefficients with session score and mean reaction time. These indicators are intended as heuristic monitoring features and longitudinal summary measures, not as validated diagnostic biomarkers or predictive clinical models.
...
Because its analytics layer is based on interpretable statistical indicators and its task-planning workflow is explicitly defined, NeuroBloom also supports transparent, reproducible, and extensible digital monitoring research.

\section{Conclusions}
This paper presented NeuroBloom as a web-based platform for supporting repeated cognitive follow-up and clinician-supervised rehabilitation activity in multiple sclerosis. Its main contribution lies in bringing together task-based assessment, contextual session information, analytics-driven review, and multi-role system support within a single coordinated environment. Rather than functioning as a diagnostic instrument, NeuroBloom is intended to facilitate structured monitoring, rehabilitation-oriented decision support, and the study of behavioral change across time. Future work will focus on broader deployment, expansion of task coverage, refinement of analytical measures, and further evaluation in research and supervised clinical settings.
