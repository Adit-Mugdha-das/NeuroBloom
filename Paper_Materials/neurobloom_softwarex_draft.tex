...
NeuroBloom is organized around a modular, role-aware architecture that separates interface functions, application services, analytics processing, and persistent storage across three operational contexts—patient, doctor, and administrator—which share a common backend and data model while maintaining role-specific workflows.
...
Bilingual support for Bangla and English is implemented as a custom internationalisation layer within the frontend. Locale preference is stored in the user account record and bootstrapped from a \texttt{data-locale} HTML attribute before client-side hydration, avoiding layout shift at page load. A structured translation catalog handles interface strings through exact-match, phrase-level, and word-level substitution tiers. Numeric outputs are rendered using Unicode Bengali digits when Bangla is active, and Latin letter stimuli used in cognitive tasks are remapped to corresponding Bengali script characters to preserve script consistency without altering task structure or scoring logic. Bilingual support is scoped to the patient-facing interface; clinician and administrative views are presented in English.
...
Data flow begins when a patient completes baseline or training activities and optionally submits contextual information such as fatigue, sleep quality, stress, or medication timing. Stored session records—including score, accuracy, response time, completion behavior, and contextual variables—are accessible to clinicians and administrators, allowing performance to be reviewed across sessions rather than as isolated results.

An analytics layer operates on these stored records as a monitoring pipeline, deriving variability- and fatigue-related measures, evaluating change across sessions, and returning interpretable summaries for clinician-facing review rather than acting as a standalone diagnostic engine. The processing sequence is illustrated in Fig.~\ref{fig:biomarker_pipeline}.
...
Beyond raw task completion, the platform derives summary analytics from accumulated performance and contextual records, including longitudinal trend summaries, fatigue-related patterns, and response-time variability indicators. These are intended to support progression-aware monitoring and clinician interpretation of rehabilitation-relevant change rather than to provide diagnostic conclusions. The processing pipeline is illustrated in Fig.~\ref{fig:biomarker_pipeline}; an example dashboard is shown in Fig.~\ref{fig:admin_workflow}(a).
