"""
Tower of London (TOL) Task Service

Clinical validation: Executive planning gold standard
Description: Move colored disks to match target configuration in minimum moves

MS Research Evidence:
- Planning and problem-solving measure
- Correlates with prefrontal cortex function
- Reference: Shallice, 1982; MS cognitive studies

Three pegs with capacity constraints:
- Peg 1: Holds 3 disks
- Peg 2: Holds 2 disks
- Peg 3: Holds 1 disk

Difficulty scales by minimum moves needed (2-6+ moves)
"""
...
class TowerOfLondonTask:
    """
    Tower of London implementation
    
    Classic planning task with 3 colored disks on 3 pegs
    User must rearrange disks to match goal state in minimum moves
    ...
    # Peg capacities
    PEG_CAPACITIES = [3, 2, 1]  # Peg 0 holds 3, Peg 1 holds 2, Peg 2 holds 1
    ...
    DIFFICULTY_CONFIG = {
        ...
        10: {
            ...
            "max_moves": 7,
            "problems_count": 10,
            "planning_time_seconds": 15,
            "move_penalty": True,
            "show_minimum_moves": False,
        },
    }
    
    # Pre-defined problem sets organized by minimum moves
    # Each problem is (start_state, goal_state, min_moves)
    PROBLEM_LIBRARY = {
        2: [
            # 2-move problems
            ([[0, 1, 2], [], []], [[1, 2], [0], []]),
    ...
    def generate_session(cls, difficulty: int = 1) -> Dict[str, Any]:
        """
        Generate a complete TOL session with multiple problems
        ...
        for i in range(config["problems_count"]):
            ...
            if target_moves in cls.PROBLEM_LIBRARY:
                ...
                start_copy = [peg[:] for peg in start]
                goal_copy = [peg[:] for peg in goal]
        ...
        session_id = f"tol_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{random.randint(1000, 9999)}"
        
        return {
            "session_id": session_id,
            "task_type": "tower_of_london",
            ...
            "config": {
                ...
                "peg_capacities": cls.PEG_CAPACITIES,
                "colors": cls.COLORS
            },
            "problems": problems,
    ...
    def score_session(cls, session_data: Dict[str, Any],
        ...
        expected_time_per_problem = config.get("planning_time_seconds", 30) * 0.5  # Expect to use half the planning time
...
# Singleton instance
tol_task_service = TowerOfLondonTask()
